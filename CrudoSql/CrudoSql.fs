...
module String = 
    let truncate count (s: string) =
        ...
        if len > count then 
            s.[0..(min count (String.length s)-1)] + "..."
        else
            s
...
let renderTable (cols: ColSpec[]) (outputTableCols: ColRef[]) (readResult: TableReadResults) =
    ...
    let renderCell (spec: ColSpec) (cell: obj) (summaries: Map<ColRef, obj>) = 
        match spec with
        | Col(s) | Summ(s) -> Any cell
        | Fk (colname,fk) -> 
            let (ColRef(tab, col)) = fk.Target
            let summaryCol = fk.Summarizer |> function | ColRef(_, col) -> ColRef(Table (joinedTableName colname), col)  
            let summarized = Map.tryFind summaryCol summaries
            //let summarized = Map.tryFind join summaries
            let summ2 = ()
            let value = match summarized with 
                        | Some s -> sprintf "%s (%s)" (s.ToString()) (String.truncate 5 (cell.ToString()))
                        | None -> cell.ToString()
            Link((sprintf "/table/%s?%s=%s" tab.Name col (cell.ToString())), value)
        | Prim(s) -> Link(LinkGen.TableSearch tableName s (cell.ToString()), cell.ToString())
    ...
    seq {
        for row in table.Rows do
            ...
            let summaryDict =
                Array.zip (outputTableCols.[cols.Length..]) (row.[cols.Length..]) 
               |> Map.ofSeq
...
let readTable tname (isRaw: bool) (req: HttpRequest) =
    ...
    let tab = if isRaw then None else schema.TryFindTable tname
    let src = Table tname

    let tableHints = if tab.IsSome then tab.Value.Hints |> Array.ofSeq else [||]
    
    let fillRemaining = tableHints |> Array.exists (function | FillAll -> true | _ -> false) 

    let inputCols =
        if tab.IsNone then [||] else
            if fillRemaining then
                let ix = Introspect(db)
                let toFill = 
                    ix.ColTypes src
                    |> Map.toSeq
                    |> Seq.map fst
                    |> Seq.filter (fun col -> if Option.isSome (tab.Value.TryGetCol col) then false else true)
                    |> Seq.map ColSpec.Col
                    |> Array.ofSeq
    ...
    let foreignKeys (tab: TableSpec) =
        //tab.ForeignKeys
        inputCols |> Array.choose (function | Fk(n, fk) -> Some (n,fk) | _ -> None)
    ...
    let joins = fks 
                |> Seq.map (fun (n,fk) -> JoinOn (fk.Target, src.Col(n), Table (joinedTableName n), "LEFT OUTER"))
    
    let summarizers = fks 
                      |> Seq.map (fun (n, fk) -> match fk.Summarizer with 
                                                 | ColRef((Table tname), colname) -> ColRef(Table (joinedTableName n), colname))                                                        
                      |> Array.ofSeq
    
    let outputTableCols = match tab with 
                          | Some spec -> 
                                Array.concat [
                                    inputCols |> Array.map (fun n -> src.Col n.Name)
                                    summarizers
                                ]
                                |> Some
                          | _ -> None 
    ...
    let aliasColumns (cols: ColRef[]) =
        ...
        cols |> Array.map (fun col ->
                           ...
                           let tabprefix = if tab = mainTable then "" else (tab.Replace("JOINED_", "") + "_")
        ...
        | Some spec ->
            let sortcol = 
                ...
                |> (fun sorterspec ->
                        ...
                        | Fk (_, fk) -> sprintf "%s.%s" (joinedTableName (sorterspec.Name)) fk.Summarizer.Right  
                        | _ -> (spec.Ref.Col sorterspec.Name).Str 
                        )
            let sorterColumnIndex = outputTableCols.Value |> Array.findIndex (fun cref -> cref.Str = sortcol)        
            (outputTableCols.Value |> aliasColumns |> Seq.ofArray |> SelectAs, OrderBy [sprintf "%d ASC" (sorterColumnIndex+1)])
        | _ -> (SelectS ["*"], OrderBy ["1"])
    ...
    let pager = match filters with
                | _ when Array.contains (Meta("$limit","off")) filters -> Skip
                | _ when pagerSkip.IsSome && pagerLimit.IsSome -> Page (int pagerSkip.Value.Value) (int pagerLimit.Value.Value)
    ...
    if data.Rows.Length >= MAX_PAGE_SIZE || pagerSkip.IsSome then
        ...
        let countres = SqlRunner.Run (req.url.PathAndQuery + " count") countClause
    ...
    | None -> 
        View.TableRaw data tname filters readResult
...
let saveRow tableName ctx =
    ...
    let quoteIfNeeded colName value =
        ...
        | Some (Num _) -> value
        | Some (Str _) | Some (Unk _) -> sqlQuoted value       
        | _ -> sqlQuoted value
    
    let action = received |> List.find (fun el -> fst el = "action") |> snd |> Option.get
    ...
    let identityinsert tab mode = sprintf "set IDENTITY_INSERT %s %s" tab mode |> Raw 
    let stmt = match action with
                | "update" -> 
                    [
    ...
    SqlRunner.Run (sprintf "%s %s" action ctx.Req.path) stmt
    |> ignore

let queryOnTable tableName = request (fun r ->
    OK (readTable tableName false r))

let queryOnTableRaw tableName = request (fun r ->
    OK (readTable tableName true r))

let queryOnEditRow tableName = request (fun r ->
    let ctx = CreateViewContext r
    OK (editTable tableName ctx))

let queryOnSaveRow tableName = request (fun r ->
    let ctx = CreateViewContext r
    saveRow tableName ctx
    Redirection.redirect <| LinkGen.SearchWithFilters "editrow" (Table tableName) ctx.Filters
)
...
let showIndex() = 
    let conndesc = sprintf "Connection: %s" <| Db.DbConnector.DefaultConnector.Show()
    View.Index(SchemaGen.GetSpec(), conndesc)    
    |> OK

let sqlLog() = 
    View.SqlLog <| SqlRunner.RequestLog
    |> OK
...
let app =
    choose
      [     
          ...
          GET >=> path "/alltables/" >=> request (fun _ -> OK (viewAllTables()))
