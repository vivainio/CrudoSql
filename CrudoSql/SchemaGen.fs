...
let decl (tab: Table) (cols: ColSpec seq) =
    declWith tab cols |> ignore
...
let debugCol col = 
    let dumper: DeferFunc = fun ts cs -> 
...
let sortCol col =
    let addSortBy: DeferFunc = fun ts cs ->
...
let prettyCol (prettyFunc: string -> string) col =
    let addFilter: DeferFunc = fun ts cs ->
        ValueFilter(cs.Name, prettyFunc) |> ts.Hints.Add
        None
    Defer(col, addFilter)

let hint (hint: TableHint) (ts: TableSpec) =
    ts.Hints.Add (hint)
