...
module DbConnector =
    ...
    type Connector(provider: string, connectionString: string) = 
        ...
        member x.Syntax() = if provider.Contains "SqlClient" then SqlSyntax.Any else SqlSyntax.Ora
    ...
    let mutable DefaultConnector = 
        Connector(MsSql, ConnectionString [ DataSource "localhost"; Catalog "IA"; IntegratedSecurity true])
...
with member x.Val = match x with 
                    | Any o -> o.ToString()
                    | Link(_, label) -> label
...
with
    ...
    member x.Prim = x.Cols |> Array.pick (function | Prim n -> Some n | _ -> None)
    member x.Summ = x.Cols |> Array.tryPick (function | Summ n -> Some n | _ -> None) |> Option.defaultValue x.Cols.[0].Name
    member x.ForeignKeys = x.Cols |> Array.filter (function | Fk _ -> true | _ -> false)
    member x.GetCol name = x.Cols |> Array.find (fun col -> col.Name = name)
    member x.TryGetCol name = x.Cols |> Array.tryFind (fun col -> col.Name = name)
...
type DbSpec() = 
    ...
    member x.TryFindTable name = x.Tables |> Seq.tryFind (fun t -> t.Name = name)
    member x.ReferringForeignKeys (toTable: Table) = 
        x.Tables 
        |> Seq.choose (fun t ->
            let fksToHere = t.ForeignKeys
                            |> Array.filter (function
                                          | Fk(_, fk) when fk.Target.Table = toTable.Name -> true
                                          | _ -> false)
            if Array.isEmpty fksToHere then None else Some (t, fksToHere)
            )
        |> Seq.toArray
...
type Conn() =
    ...
    member x.QueryImpl q =
        ...
        let headers = [| 0..(reader.FieldCount-1) |] |> Array.map (fun i -> reader.GetName i)
...
let RunSql (query: Frag seq) =
    let conn = Conn()
    query |> serializeSql SqlSyntax.Any |> conn.Query "" |> fun resp -> (resp.Rows, resp)

let cast2<'a, 'b> (objs: obj[]) =
    (objs.[0] :?> 'a, objs.[1] :?> 'b)
...
let Test() =
    ...
    let q = [
                SelectS ["*"]
                FromS ["INFORMATION_SCHEMA.TABLE_CONSTRAINTS Tab"; 
    ...
    let res = db.Query "" q
    let testq q = 
        q |> serializeSql SqlSyntax.Any |> fun s -> printfn "%s" s; s |>  db.Query "" |> printfn "%A"
